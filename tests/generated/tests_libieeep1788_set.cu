...
void tests_libieeep1788_set_call(int numBlocks, int blockSize, cuda_buffer buffer, cudaStream_t stream,
                 int n, interval<double> *d_xs, interval<double> *d_ys, interval<double> *d_res)
                 {
                test_libieeep1788_set<<<numBlocks, blockSize, 0, stream>>>(n, d_xs, d_ys, d_res); 
            }