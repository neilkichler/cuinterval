...
Since most math functions do not have corresponing operations that round towards ±∞, we make use of nextafter(x, ±∞). Sometimes, we know that the function is bounded (e.g., $$\exp(x) > 0$$). In those cases, we apply `nextafter(x, 0)` or similar tighter outward roundings whenever possible.
:::
