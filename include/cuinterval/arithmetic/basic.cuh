...
template <typename T> __device__ interval<T> pos_inf() {
  return {intrinsic::pos_inf<T>(), intrinsic::pos_inf<T>()};
}

template <typename T> __device__ interval<T> empty() {
  return {intrinsic::nan<T>(), intrinsic::nan<T>()};
}

template <typename T> __device__ interval<T> entire() {
  return {intrinsic::neg_inf<T>(), intrinsic::pos_inf<T>()};
}

template <typename T> __device__ bool empty(interval<T> x) {
  return isnan(x.lb) || isnan(x.ub);
}

// Basic operations

template <typename T> __device__ interval<T> neg(interval<T> x) {
  return {-x.ub, -x.lb};
}

template <typename T> __device__ interval<T> add(interval<T> a, interval<T> b) {
  return {intrinsic::add_down(a.lb, b.lb), intrinsic::add_up(a.ub, b.ub)};
}

template <typename T> __device__ interval<T> sub(interval<T> a, interval<T> b) {
  return {intrinsic::sub_down(a.lb, b.ub), intrinsic::sub_up(a.ub, b.lb)};
}

template <typename T> __device__ interval<T> mul(interval<T> a, interval<T> b) {
  interval<T> c;
  c.lb = min(
...
template <typename T> __device__ interval<T> sqr(interval<T> x) {
  if (x.lb >= 0) {
...
template <typename T> __device__ interval<T> sqrt(interval<T> x) {
  return {x.lb <= 0 && x.ub > 0 ? 0 : intrinsic::sqrt_down(x.lb),
          intrinsic::sqrt_up(x.ub)};
}

template <typename T> __device__ bool contains(interval<T> x, T y) {
  return x.lb <= y && y <= x.ub;
}

template <typename T> __device__ interval<T> recip(interval<T> a) {
...
template <typename T> __device__ interval<T> div(interval<T> x, interval<T> y) {
...
template <typename T> __device__ interval<T> mag(interval<T> x) {
  return max(abs(x.lb), abs(x.ub));
}

template <typename T> __device__ interval<T> mig(interval<T> x) {
...
template <typename T> __device__ T rad(interval<T> x) {
  if (empty(x)) {
...
template <typename T> __device__ interval<T> abs(interval<T> x) {
  return {mig(x.lb), mag(x.ub)};
}

template <typename T> __device__ interval<T> max(interval<T> x, interval<T> y) {
  return {max(x.lb, y.lb), max(x.ub, y.ub)};
}

template <typename T> __device__ interval<T> min(interval<T> x, interval<T> y) {
  return {min(x.lb, y.lb), min(x.ub, y.ub)};
}

template <typename T> __device__ interval<T> operator+(interval<T> x) {
  return x;
}
...
template <typename T> __device__ bool entire(interval<T> x) {
  return intrinsic::neg_inf<T>() == x.lb && intrinsic::pos_inf<T>() == x.ub;
}

template <typename T> __device__ bool bounded(interval<T> x) {
  return !entire(x) || empty(x);
}

template <typename T> __device__ T width(interval<T> x) {
  if (empty(x))
    return {};
...
template <typename T> __device__ T inf(interval<T> x) { return x.lb; }

template <typename T> __device__ T sup(interval<T> x) { return x.ub; }

template <typename T> __device__ T mid(interval<T> x) {
  // if (x.lb == x.ub) {
  //     return x.lb;
  ...
  return (x.lb == x.ub) * x.lb +
         (abs(x.lb) != abs(x.ub)) * (0.5 * x.lb + 0.5 * x.ub);
}

template <typename T> __device__ bool equal(interval<T> a, interval<T> b) {
  return (empty(a) && empty(b)) || (a.lb == b.lb && a.ub == b.ub);
}

template <typename T> __device__ bool subset(interval<T> a, interval<T> b) {
  return empty(a) || ((a.lb <= b.lb) && (b.ub <= a.ub));
}

template <typename T> __device__ bool interior(interval<T> a, interval<T> b) {
  return empty(a) || ((a.lb < b.lb) && (b.ub < a.ub));
}

template <typename T> __device__ bool disjoint(interval<T> a, interval<T> b) {
...
template <typename T> __device__ interval<T> ceil(interval<T> x) {
  return {intrinsic::int_up(x.lb), intrinsic::int_up(x.ub)};
}

template <typename T> __device__ interval<T> floor(interval<T> x) {
  return {intrinsic::int_down(x.lb), intrinsic::int_down(x.ub)};
}

template <typename T> __device__ interval<T> trunc(interval<T> x) {
